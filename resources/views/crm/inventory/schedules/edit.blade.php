...
@section('title', 'Норма выдачи для: ' . $item->name)
@section('content')
                    ...
                    <div class="col-lg-8 col-12">
                        <div class="page-right-content">
                            <div class="service-title">
                                <h2>{{ 'Норма выдачи для: ' . $item->name }}</h2>
                            </div>

                            <form action="{{route("crm.inventory.schedule.update", [$item, $schedule])}}" method="POST"
                                  class="account-form">
                                @csrf @method('PUT')
                                            ...
                                            <select name="profession_id" id="profession_id"
                                                ...
                                                @foreach($professions as $prof)
                                                    <option value="{{$prof->id}}"
                                                            @if($prof->id==old("profession_id", $schedule->profession_id)) selected @endif>
                                                        {{$prof->name}}
                                                    </option>
                                                @endforeach
                                            </select>
