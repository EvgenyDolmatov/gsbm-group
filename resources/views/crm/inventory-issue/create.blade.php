...
@section('title', 'Геострой-Буммаш CRM - Выдача СИЗ для ' . $employee->getFullName())
@section('content')
                ...
                <div class="row">
                    ...
                    <div class="col-xl-8 col-lg-7 col-12">
                        <div class="page-right-content">
                            ...
                            <form action="{{route("crm.inventory-issue.store", $employee)}}" method="POST"
                                  class="account-form">
                                @csrf
                                            ...
                                            <select name="inventory_item_id" id="inventory_item_id"
                                                ...
                                                @foreach($items as $item)
                                                    <option value="{{$item->id}}"
                                                            @if($item->id==old("inventory_item_id")) selected @endif>
                                                        {{$item->name}}
                                                    </option>
                                                @endforeach
                                            </select>
                                ...
                                <button type="submit" class="btn btn-brand btn-filled mb-3">Выдать</button>
                            </form>
                        </div>
                    </div>
                </div>
